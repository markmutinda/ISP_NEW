...
class RouterIncomeView(APIView):
    ...
    FIXED: Uses Payment table as single source of truth with service_type.
    service_type is written once at creation and never touched by SET_NULL
    cascades from pruning, so accuracy no longer degrades over time regardless
    of prune window.
    """
    permission_classes = [IsAuthenticated, IsAdminOrStaff, HasRoleAccessPolicy]
    required_rbac_path = "/admin/analytics"
    ...
    def get(self, request, router_id):
        ...
        # FIX 5: Durable fix using service_type — written once at creation,
        # never touched by SET_NULL cascades from pruning.
        hotspot_income = (
            Payment.objects
            .filter(status='COMPLETED', service_type='HOTSPOT', hotspot_sessions__router_id=router_id)
            .distinct()
            .aggregate(total=Sum('amount'))['total'] or 0
        )
        ...
        pppoe_income = (
            Payment.objects
            .filter(status='COMPLETED', service_type='PPPOE', customer_id__in=pppoe_customer_ids)
            .aggregate(total=Sum('amount'))['total'] or 0
        )
