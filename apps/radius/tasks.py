...
5. Auto-generating renewal invoices for expiring PPPoE users
"""
...
# ════════════════════════════════════════════════════════════════════════════
# PPPOE EXPIRY INVOICE GENERATION — Auto-renewal invoices 5 days before expiry
# ════════════════════════════════════════════════════════════════════════════

@shared_task(name='apps.radius.tasks.generate_pppoe_expiry_invoices')
def generate_pppoe_expiry_invoices():
    """
    5 days before expiry → auto-generate an invoice for the customer's current plan.
    Runs daily. Skips if invoice already exists for this customer + expiry period.
    """
    ...
    for tenant in TenantModel.objects.exclude(schema_name='public'):
        try:
            with schema_context(tenant.schema_name):
                from apps.radius.models import CustomerRadiusCredentials
                from apps.billing.models.billing_models import Invoice, InvoiceItem
                ...
                stats['expiring_found'] += expiring.count()
                ...
                for cred in expiring:
                    ...
                    plan = service.plan
                    expiry = cred.expiration_date

                    # Idempotency: skip if invoice already exists for this period
                    already_exists = Invoice.objects.filter(
                        customer=customer,
                        plan=plan,
                        service_period_end__date=expiry.date(),
                        status__in=['DRAFT', 'ISSUED', 'SENT', 'PARTIAL', 'PAID'],
                    ).exists()
                    if already_exists:
                    ...
                    invoice = Invoice.objects.create(
                        ...
                        service_period_start=expiry.date(),
                        service_period_end=(expiry + plan.get_validity_timedelta()).date() if plan.get_validity_timedelta() else expiry.date(),
                        schema_name=db_conn.schema_name,
                        status='ISSUED',
                        issued_at=now,
                        subtotal=plan.base_price,
                        total_amount=plan.base_price,
                        balance=plan.base_price,
                        notes=f'Auto-generated renewal invoice for {plan.name}',
                    )
                    ...
                    logger.info(
                        f"[INVOICE] Generated renewal invoice {invoice.invoice_number} "
                        f"for {customer.customer_code} in {tenant.schema_name}"
                    )

        except Exception as e:
            stats['errors'] += 1
            logger.error(f"[INVOICE] Error in {tenant.schema_name}: {e}")

    logger.info(f"[INVOICE] Complete: {stats}")
    return stats