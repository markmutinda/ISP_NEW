...
def monitor_vpn_tunnels():
    ...
    try:
        ...
        # Get currently connected VPN clients (shared — not tenant-specific)
        try:
        ...
        def _check_tenant(tenant):
            ...
            total = provisioned_routers.count()
            if disconnected_count:
                logger.warning(
                    f"[{tenant.schema_name}] VPN Monitor: {disconnected_count} routers disconnected: "
        ...
        result = _for_each_tenant(_check_tenant)
        result['vpn_clients_total'] = len(connected_clients)
        return result
