...
class BandwidthProfileAdmin(admin.ModelAdmin):
    list_display = ['name', 'profile_type', 'download_speed', 'upload_speed', 'data_cap', 'monthly_price', 'is_active']
...
class TrafficRuleAdmin(admin.ModelAdmin):
    list_display = ['name', 'rule_type', 'priority_level', 'is_active', 'is_applied', 'applied_at']
    list_filter = ['rule_type', 'priority_level', 'is_active', 'is_applied', 'company']
...
class DataUsageAdmin(admin.ModelAdmin):
    list_display = ['customer', 'period_start', 'period_end', 'download_gb', 'upload_gb', 'total_gb', 'usage_percentage']
    list_filter = ['period_start', 'period_end', 'is_over_limit']
    search_fields = ['customer__customer_code', 'customer__user__email']
    readonly_fields = ['download_gb', 'upload_gb', 'total_gb', 'usage_percentage']


@admin.register(BandwidthAlert)
class BandwidthAlertAdmin(admin.ModelAdmin):
    list_display = ['alert_type', 'alert_level', 'customer', 'is_triggered', 'triggered_at', 'acknowledged', 'resolved']
    list_filter = ['alert_type', 'alert_level', 'is_triggered', 'acknowledged', 'resolved', 'company']
...
class SpeedTestResultAdmin(admin.ModelAdmin):
    list_display = ['customer', 'download_speed', 'upload_speed', 'latency', 'test_time', 'is_valid']
