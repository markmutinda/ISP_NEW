...
class FUPUsageService:
    ...
    def resolve_window(self, policy, service_connection, now=None):
        ...
        if policy.reset_period == 'DAILY':
            ...
            # If expiration_date exists, use it as the end of the window
            if service_connection.expiration_date:
                end = service_connection.expiration_date
            else:
                # Fallback: if no expiration date, use 30 days from start
                # (This should not happen in normal operation)
                end = start + timedelta(days=30)
            
            # If the current time is beyond the end date, we need to find the current window
            # by incrementing until we find the window containing 'now'
            if end <= now:
                # Move windows forward until we find the current period
                while end <= now:
                    start = end
                    end = start + timedelta(days=(service_connection.expiration_date - service_connection.activation_date).days 
